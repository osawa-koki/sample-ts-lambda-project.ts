...
```bash
sample-ts-lambda-project.ts$ sam build
```
...
```bash
sample-ts-lambda-project.ts$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sample-ts-lambda-project.ts$ sam local start-api
sample-ts-lambda-project.ts$ curl http://localhost:3000/
```
...
```bash
sample-ts-lambda-project.ts$ sam logs -n HelloWorldFunction --stack-name sample-ts-lambda-project.ts --tail
```
...
```bash
sample-ts-lambda-project.ts$ cd hello-world
...
```bash
sam delete --stack-name sample-ts-lambda-project.ts
```
